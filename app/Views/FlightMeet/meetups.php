...
<?= $this->section('content') ?>
    ...
    <!-- Globale Fehlermeldungen bei fehlgeschlagenen Aktionen -->
...
<?php endif; ?>
    ...
    <!-- Fallbacks für Filter- und Status-Variablen -->
<?php $meetups = $meetups ?? []; ?>
...
<?php $statusOptions = ['geplant', 'ausgebucht', 'abgeschlossen', 'abgesagt'] ?>
    ...
    <!-- Such- und Filterformular (Regionen, Erfahrungslevel, Freitextsuche) -->
    <form class="fm-filter" method="get" action="<?= base_url('flightmeet/meetups') ?>">
        ...
        <!-- Filter-Aktionen und Status-Checkboxen -->
    ...
    <!-- Steuerungselemente: Ansichten-Umschalter (Kacheln/Tabelle) & Erstellen-Button -->
                ...
                <?php foreach ($meetups as $meetup): ?>
                    <a class="fm-card fm-card--link" href="<?= base_url('flightmeet/meetups/' . $meetup['id']) ?>">
                        <header class="fm-card__header">
                            <h3>
                                <?= esc($meetup['title']) ?>
                                <!-- Dezenter Haken bei bereits aktiver Anmeldung -->
                                <?php if (isset($meetup['is_participating']) && (int)$meetup['is_participating'] === 1): ?>
                                    <i class="ph ph-check-circle" style="color: #28a745; margin-left: 6px; font-size: 1.05rem; vertical-align: middle;" title="Angemeldet"></i>
                                <?php endif; ?>
                            </h3>
    ...
    <!-- TABELLENANSICHT (Umschaltbar, sortierbar über list.js) -->
    <section class="fm-section" id="fmTableView">
        <div class="fm-table-wrapper">
            <table class="fm-table">
                <thead>
                <tr>
                    <!-- Tabellenköpfe mit List.js-Sortierungsklassen -->
                ...
                <!-- List.js benötigt die Klasse 'list' im tbody -->
                <tbody class="list">
                <?php if ($meetups === []): ?>
                    <tr>
                        ...
                        <tr class="fm-row-link">
                            <td class="fm-row-link__cell col-title" data-title="<?= esc($meetup['title']) ?>">
                                <a class="fm-row-link__anchor" href="<?= base_url('flightmeet/meetups/' . $meetup['id']) ?>">
                                    ...
                                    <!-- Dezenter Haken bei bereits aktiver Anmeldung -->
                                    <?php if (isset($meetup['is_participating']) && (int)$meetup['is_participating'] === 1): ?>
                                        <i class="ph ph-check-circle" style="color: #28a745; margin-left: 4px; vertical-align: middle;" title="Angemeldet"></i>
                                    <?php endif; ?>
                                </a>
                            </td>
                            <td class="col-spot"><?= esc($meetup['location']) ?></td>
                            <td><span class="fm-badge-region col-region"><?= esc($meetup['region']) ?></span></td>
                            <!-- Zeitstempel für List.js-Sortierung hinterlegt -->
