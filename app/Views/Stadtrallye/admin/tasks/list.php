...
<body>
    ...
    <div class="container">
        ...
        <div class="button-group">
            <a href="<?= site_url('stadtrallye/admin/tasks/create/' . $stationId) ?>">+ Neue Aufgabe</a>
            <a class="secondary" href="<?= site_url('stadtrallye/admin/stations/' . $station['rally_id']) ?>">← Zurück</a>
        </div>
                ...
                <?php else: ?>
                    <?php foreach ($tasks as $task): ?>
                        <tr>
                            ...
                            <td><?= esc((string)($task['points'] ?? 0)) ?></td>
                            <td class="actions">
                                <a href="<?= site_url('stadtrallye/admin/tasks/edit/' . $task['id']) ?>">Bearbeiten</a>
                                <form method="post" action="<?= site_url('stadtrallye/admin/tasks/delete/' . $task['id']) ?>" style="display:inline" onsubmit="return confirm('Wirklich löschen?');">
                                    <?= csrf_field() ?>
