    ...
    <div class="container">
        ...
        <?php if (!empty($rallies)): ?>
            <div class="rally-selector">
                ...
                <select id="rally_select" onchange="window.location.href='<?= site_url('stadtrallye/leaderboard') ?>/' + this.value">
                    <option value="">-- Wählen Sie eine Rallye --</option>
                    <?php foreach ($rallies as $r): ?>
                        <option value="<?= esc((string)$r['id']) ?>" <?= ($r['id'] == ($rallyId ?? null)) ? 'selected' : '' ?>>
                            <?= esc($r['title']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                ...
                <tbody>
                    <?php foreach ($leaderboard as $index => $entry): ?>
                        <tr>
                            ...
                            <td><strong><?= esc((string)($entry['score'] ?? 0)) ?></strong></td>
                            <td><?= esc((string)($entry['submissions_count'] ?? 0)) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
