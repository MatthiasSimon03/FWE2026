...
class Auth extends BaseController
{
    ...
    public function register()
    {
        if ($this->request->is('post')) {
            ...
            if ($this->userModel->createUser($name, $email, $password)) {
                return redirect()->to(site_url('auth/login'))->with('success', 'Registrierung erfolgreich! Bitte melden Sie sich an.');
            }
    ...
    public function login()
    {
        if ($this->request->is('post')) {
            ...
            if ($user) {
                ...
                return redirect()->to(site_url('rally'))->with('success', 'Anmeldung erfolgreich!');
            }
    ...
    {
        session()->destroy();
        return redirect()->to(site_url('rally'))->with('success', 'Abmeldung erfolgreich.');
    }
}
