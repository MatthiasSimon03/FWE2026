...
class Stations extends \App\Controllers\BaseController
{
    ...
    public function index($rallyId = null)
    {
        ...
        if (!$rallyId) {
            ...
            if (empty($rallies)) {
                return redirect()->to(site_url('admin/rallies'))->with('error', 'Es gibt keine Rallyen.');
            }
            return redirect()->to(site_url('admin/stations/' . $rallies[0]['id']));
        }
        ...
        return view('admin/stations/list', ['rally' => $rally, 'stations' => $stations, 'rallyId' => $rallyId]);
    }
    ...
    public function create($rallyId)
    {
        ...
        if ($this->request->is('post')) {
            ...
            return redirect()->to(site_url("admin/stations/$rallyId"))->with('success', 'Station erstellt.');
        }

        return view('admin/stations/create', ['rallyId' => $rallyId]);
    }
    ...
    public function edit($stationId)
    {
        ...
        if ($this->request->is('post')) {
            ...
            return redirect()->to(site_url('admin/stations/' . $station['rally_id']))->with('success', 'Station aktualisiert.');
        }
    ...
    public function delete($stationId)
    {
        ...
        return redirect()->to(site_url('admin/stations/' . $rallyId))->with('success', 'Station gelöscht.');
    }
}
